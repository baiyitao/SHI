...
int main() {
  int n;
  printf("Seq matrix multiply 1-1000\n");
  printf("n,time,Mflops\n");

  double Mflop;

for(n = 10; n < 1000; n+=10){
  Mflop = 2*n*n*n*1.0/1000000;
  ...
  for(int i = 0; i < n; i++) {
    for(int j = 0; j < n; j++) {
	  ...
	  r1=rand()%1000;
    r2=rand()%1000;
      matrix_index_set(A, i, j, r1);
      matrix_index_set(B, i, j, r2);
    }
  }
  ...
  double time_x = calc_time(&start, &stop);
  printf("%d,%lf,%lf\n", n,time_x, Mflop/time_x);
